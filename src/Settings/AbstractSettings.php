...
abstract class AbstractSettings implements \JsonSerializable {
   ...
   * Enum properties.
   *
   * Map from name to Backed enum type, e.g.
   *
   * <code>
   *   $enumProperties = [
   *     'type' => MyType::class,
  ...
  public function apply(array $values, bool $throwExceptionOnMissingProperty = FALSE): static {
    ...
    foreach (static::$enumProperties as $property => $class) {
      if (isset($values[$property]) && is_scalar($values[$property])) {
        $values[$property] = $class::tryFrom($values[$property]);
      }
    }
